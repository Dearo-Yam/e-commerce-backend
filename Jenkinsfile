    ...
    stages {
        ...
        stage('Docker Build Image and Push') {
            steps {
                ...
                withCredentials([usernamePassword(credentialsId: 'wms_backend_pwd',
                usernameVariable: 'USERNAME', passwordVariable: 'PASSWORD')]) {
                    sh """
                    docker login --username $USERNAME --password $PASSWORD
                    docker build -t $USERNAME/wms_backend:${env.BUILD_NUMBER} .
                    docker push $USERNAME/wms_backend:${env.BUILD_NUMBER}
